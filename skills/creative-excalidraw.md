...
### Rendering Diagrams as PNG (Alternative)

If you need a PNG screenshot of a diagram:
- **Excalidraw URL approach is unreliable** — playwright cannot load Excalidraw from JSON URLs reliably (data gets lost on redirect)
- **Better approach**: Use `matplotlib` to render the diagram directly as PNG. See `references/matplotlib_diagram.md` for a template.
- If you must use Excalidraw in browser: open playwright → navigate to the URL → screenshot. The URL must be a full direct link (not a JSON data URL).
